...
class MCTF:
    ...
    def backpropagation(self, node_id:tuple, win:int):
        while not node_id == (0,):  # if not root node?
            ...
            # self.tree[node_id]['visit'] += 1; self.tree[node_id]['win'] += win  # 메모리가 가르키는 위치가 같음. 두번적용됨
            # self.update_node(node)
...
#<<<<<<<<<<<<<<<<<<<<<<< 2023/4/6 12/26 여기까지 변경함 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,,
    def node_drop(self, node_id:tuple):  # 자식 노드를 드랍  # 부모 노드 승률 고려? <<< 고민 필요
        ...
        if not False in winning_list and len(winning_list) > 1:
            _id = child_node_id[winning_list.index(min(winning_list))]  # _id : 제거 대상 node_id 

            # print('Drop : \n{} : {}'.format(child_node_id[n], self.tree[child_node_id[n]]))  # <<<
            # print(_id)
            node['child'].remove(_id)  # <<<<<<<< self.tree[node_id]['child'] 와 같은 메모리 위치를 가르켜서 한번만 제거하면 됨.
            # self.tree[node_id]['child'].remove(_id)
            # self.tree[node['node_id']]['child'].remove(_id) # <<<<<<<<<<<<<<
            # print(node['child'], self.tree[node_id]['child'])
            ...
            if not childs_list == []: 
                ...
                for node in childs_list:  # node : tuple #node_id
                    self.remove_node(node_id=node)
                    self.tree.pop(node)
    ...
    def save_all(self):
        ...
        for node_id in self.tree:
            # print(f'save node : {node_id}')
            self.update_node(self.node_search(node_id))
        print(f'save {len(self.tree)} node')
